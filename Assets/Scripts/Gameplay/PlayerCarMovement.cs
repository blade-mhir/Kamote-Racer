...
// TIME WARP POWER-UP
    ...
    private float savedTimeSurvived = 0f;
    private float realWorldTimeSurvived = 0f;
    ...
    void Update()
    {
        ...
        realWorldTimeSurvived += Time.deltaTime; 

        // If the car is active, calculate the score as before
        if (gameObject.activeSelf)
        { 
            realWorldTimeSurvived += Time.deltaTime; 
            int score = (int)realWorldTimeSurvived * 10; 
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (collision.gameObject.CompareTag("JetFighterPowerUp"))
        {
            ...
            // Update the Jet Fighter counter
            powerUpCounter.IncrementJetFighterCounter();
            ...
            if (jetFighterPowerUpsCollected >= jetFighterPowerUpsNeeded) 
            {
                ...
                StartCoroutine(ActivateJetFighter()); 
                 // Reset the Jet Fighter counter
                powerUpCounter.ResetJetFighterCounter();
            }
   ...
   void Respawn()
    ...
    // Function to call before deactivating the car (from ActivateJetFighter)
    public void PauseScoring() 
    {
        savedTimeSurvived = timeSurvived;
    }
